...
## 2026-09-14 — more room, readable care, continuous landscape

New generated N=192; S stays pinned at 1.5. Saved and baked N are untouched. Opening placement reserves the largest founding footprint + C.FOUND_CLEAR before assigning the recipe; all 14 works are preserved. Founders scatter with dry-ground, inJar and separation checks. A late wider search handles narrow dry shelves without relaxing clearance. No new persistent fields.

survey.js is optional UI-only navigation (M, H, click/drag map, care and construction shortcuts). Camera exploration owns panHold=Infinity until H returns to the colony. Do not pull exploration back on a timeout. The survey canvas MUST override the global canvas position:fixed rule. UI rain forecast uses C.CLOUD * s.area.

landscape.js builds a ring with no triangles inside the playable square. Its first row matches the actual boundary vertices; view._surfaceY and _heightAt no longer flatten the terrain to EDGE_Y. The ring uses the real texture at the seam, blending outward into independent meadow detail. No stretched edge-pixel skirt. Shader identifier 'patch' is reserved in GLSL; use landPatch. post.blurMax=0 skips the defocus blur passes and retains bloom. Existing settings still switch the grade on/off.

Blender CLI source: tools/build-nature.py and tools/nature-kit.blend; output assets/nature.glb (107KB, no required extensions). Named oak/pine/rock pieces: eight material primitives instanced by landscape.js, plus one grass batch. Fallback forest is hidden only after the kit loads. New placements avoid starting works and railway ballast. Renderer remains sim-read-only.

Validation: 16 original sim regressions green; expanded checker: 18 openings (N96/144/192), four 60-day colonies surviving (104/47/90/71 at last checkpoint), full-save round trips and continued deterministic input, Keswick save. Browser: 19 checks, map/care interactions, held exploration/H, weather scaling, no sim writes, mobile bounds, N96 baked boot, zero script/shader errors. Boundary height difference below 1e-8; no ring vertices inside the square. Full 124-test battery subsequently passed; see continuation below. Tests support --filter with lazy weave/memory fixtures. Tilt now compares water's centre of mass against an identical level control at three sizes; the former before/after left-half fraction was confounded by weather.


### September 14 continuation — observation, responsiveness, mature towns

Follow/close-up: view.followKin stores the birth stamp and named identity; manual movement, H or touching the town releases follow. Inspector actions sit outside the regenerated body. Book names verify nameId before navigating; the living census pages through 40 names at a time. Survey cycles unfinished works in stable progress/id order. Alt/middle drag rotates without invoking a power.

clock-step.js gives ordinary frame stepping an 8ms budget with bounded backlog. It may deliver less than selected fast-forward speed on slow hardware but never changes sim tick order. catchUp uses yielding 8ms slices within the existing 2.5s total limit; catchingUp blocks ALL frame-side simulation writes as well as regular stepping. Full state equals uninterrupted stepping across a 990-tick browser comparison. HUD/weather update only on content changes; inspector updates at 5Hz. Open modals and focused button activation block power shortcuts.

nature-culling.js keeps nine material batches and checks individual plant bounds before filling instance buffers. Construction clearance refreshes every half-second; terrain revisions update plant heights. Candidate generation uses immutable terrain and stable seeded draws so new buildings do not reshuffle the forest on reload. Outer terrain refreshes when an edge is sculpted. Retina post buffers use drawing-buffer dimensions, not CSS pixels.

Sim service optimization: above 32 works, _kin builds 16-cell buckets of LIVE work references in original works order. Do not filter out incomplete works when building buckets: an earlier kin can complete one in the same pass. Rings are global candidates because their construction site moves. Final distance/progress/stock predicates are unchanged. Derived buckets are never serialized. Old/new full saves matched for five days each at N96, N192 and a mature 424-kin / 118-work town. Mature comparison: 9.67s original vs 5.15s optimized for 4500 ticks (~47% lower time); small towns keep the linear scan.

Verified: mature 180-day fixture (424 alive, 118 works), follow/close camera and cancellation, modal/keyboard guards, held-gesture readout, DPR2 at 1800x1400, book identity guards, 131 census names across four pages, construction cycling, tree clearance and terrain height, stable 7156 plant-part positions after construction/save/reload. Off-camera culling reduced submitted town triangles from 886,968 to 428,332 in the fixed test pose (geometry count, not an FPS claim). Final full-battery result: ALL 124 tests passed, zero failures or stderr errors. Independent inclusive ranges cover 1-31, 32-62, 63-93, 94-102, 103-112 and 113-124; the test-name aggregator confirmed no omissions. Includes two untouched 400-day colonies, a 300-day farming town, four 112-day soaks, later-age effects, walls and refounding. Six targeted browser scripts also pass. Local preview: http://localhost:8460/. No deployment performed; existing user saves were not edited.

Panel docking now measures the toolbar with ResizeObserver and keeps the inspector and gesture readout above wrapped rows. Ten layouts passed (1440x1000, 900x700, 390x844, 360x640, 844x390; normal and 1.35x text), with 10-11px clearance and clickable follow controls.

Map/inspector behavior: an open map temporarily hides the inspector; selecting a resident closes the map. Care shortcuts copy their guidance into #inspectHint so it stays with the resident. Selection clears old hints. Desktop and phone transitions are verified. Browser readiness checks must use window.__G?.app?.view?.natureRoot because the app exists briefly before async view setup finishes.


### September 14 graphics continuation — Blender village, water, dark gradients

This continuation is view-only; no further sim.js changes. New tools/build-village-details.py produces tools/village-details.blend and assets/village-details.glb (93,056 bytes, five material primitives). Named hut has limewashed walls, timber uprights, stone footings/step, recessed doorway/windows and a layered 32-segment straw roof. view.js loads this independently and rebuilds existing kind-3 views when ready; the original civic/procedural fallback still works. Night window positions follow the new front. Both Blender scripts disable backup .blend1 creation.

Pines now use six irregular branch skirts with alternating pointed tips rather than solid cone tiers. Rebuilt nature.glb is 116,020 bytes; still eight mesh material primitives and nine runtime instance batches including grass. The .blend sources are editable; rebuilding uses Blender 5.1 background CLI.

water-finish.js installs animated wave normals and broken specular lines on the real water surface. The ground samples the same water texture for damp-bank color/roughness, so poured pools get the effect. No decorative water affects sim levels. Water opacity and shallow/deep colors are clearer, ambient/fill lighting is gentler and key normalBias scales with cs.

Night contour bands persisted with shadows disabled and with a much weaker bump map. The fix is higher precision in post.js: HalfFloat render targets when EXT_color_buffer_float exists, UnsignedByte fallback otherwise. MSAA count is chosen from supported RGBA16F sample counts, up to four. Blur intermediates omit depth buffers. Original ground bumpScale .012 is preserved. No defocus was reintroduced. HDR requires more color-buffer memory; it is not an FPS claim.

Verified this pass: tools/verify-graphics.cjs renders Boulder and generated worlds at day/close/night, confirms all seven starting hut views use the kit and nine nature batches remain, compares full serialized simulation around rendering, and checks complete HDR and forced fallback framebuffers. Both paths returned no script/shader errors. 19 browser checks passed; landscape-life checks passed (construction clearance/return, terrain/seam update, identical 990-tick catch-up, four census pages). Retina/book regression passed at DPR2 1800x1400. The 124 simulation tests recorded above belong to the prior implementation pass and were not rerun for these visual changes. Screenshots and diagnostics are in ignored output/.

Local server remains http://localhost:8460/. The user's visible URL included newgame=1: remove that parameter before a refresh intended to load a saved colony. QA runs use isolated browser profiles and did not edit the user's save. No deployment performed.


### September 14 — little lives, inherited strengths, interaction and opening menus

User explicitly requested more creature/world/player interactions, evolution and a menu redesign. That request supersedes the old five-verbs-only restriction. The town remains autonomous and there is no score, morality currency or production queue.

SIM: C.LIFE_* constants control a 30-tick social pass. Spatial bins bound local encounters to 2.6*S; at most 64 neighbour candidates per creature. Persistent friend slots use lifeUid identity stamps, not bare recyclable kin ids. Strong bonds bias the existing company decision toward a familiar creature. Sharing transfers at most .025 FOOD NEED between a fed giver (> .78 food, > .55 water) and a hungry neighbour (< .4 food); it never creates food. Held kin, eggs, dangerous sites and emergency/summoned goals do not participate. Nearby adults can teach HALF-stage children one existing practice at a time. Play adds bounded company, strengthens bonds and shows a small hop and local colored ring.

Three new k.aptitude entries per creature: forage / empathy / resilience. Founders vary around .5; each child takes its parents’ midpoint plus a bounded .06 mutation, clamped 0..1. A separate rngLife stream avoids consuming weather or genome draws. Forage scales meal benefit by at most +/-8%; resilience scales strain by at most +/-8% and never extends lifespan; empathy affects the small companionship gain. These are heritable strengths, not acquired skills magically inherited. Existing genetics and cultural practices continue unchanged in structure.

Foragers retain 2.5% of a real moss mouthful in a seedPouch capped .035, deducting that share from their meal. The slow pass deposits up to .008 in damp depleted unpaved cells while doing something other than eating. No seed deposit on water, hot/cold danger, or worn paths. Moisture under an occupied leaf is retained slightly; leaves grant bounded shelter and rest, not food or immortality.

Player objects: Sim.placeLittle('bead'|'leaf',x,y), tidyLittle(x,y). At most 12; reject invalid coordinates, dangerous/wet ground, building footprints, neighbours within 4*S, and the outer unreachable rim. New goals 15 (play) and 16 (leaf rest) release when food/water runs low or their object disappears; existing danger overrides still apply. Objects expire at 120/18 days. They do not occupy construction footprints in the sim; future building/terrain changes remain possible. Visuals follow real terrain through cellToLocal; little-life-view.js owns their shared geometry/materials and view-only effects. No new GLB asset needed for these small props.

SAVE: root life stores identity/object sequences, props and event counts; rngLifeState stores the separate RNG cursor. New typed arrays round-trip automatically: lifeUid, friend, friendUid, bond, aptitude, socialAct, socialUntil, seedPouch. _spawn resets all of them; missing arrays use neutral constructor defaults and alive legacy kin get fresh identity stamps. The fingerprint includes root life, RNG and all new per-kin state. Full JSON equality and continued identical stepping are tested. A real IndexedDB browser reload preserved props, visible object, friendship, seed cargo and every aptitude; tests use isolated profiles.

UI: interaction-menu.js reparents the original stable buttons instead of rebuilding them under the pointer. Care/Play/Shape/Disrupt tabs only browse; picking an action retains normal arm() behavior. Tool instructions remain visible. Bead/leaf/put-away route through real pointer events. Inspector reports current social activity/friend and the three strengths; book tab 'little lives' reports encounters and mean traits by living generation. New CSS keeps inspector below the map and above the actual menu, with a sticky close header on small screens. Title screen is now a clear Enter/Continue card with town summary and Settings & controls. World replacement remains secondary and now asks before replacing an existing save. Title keyboard activation no longer enters the world through a focused settings button; settings overlay sits above the title.

VERIFIED so far: 18 focused simulation checks; generated N96 for 65-day ticks (99 alive, 89 born, generation 5, 2312 bead visits); generated N192 for 45-day ticks (38 alive, 24 born, generation 4, 1882 bead visits); Boulder for 65-day ticks (80 alive, 71 born, generation 5, 2229 bead visits). Day counters report 64/44/64 because of the existing floating day fraction; each run executes the stated days * 900 ticks. All three have full-save equality, finite fields/kin and autonomous sharing/play/teaching/seed spreading. 16 menu layouts, 10 inspector layouts (including 1.35x text), title keyboard/mobile/world-replacement checks, high-DPI journal navigation, actual pointer placement/removal and real save reload pass with no script/shader errors. Old 424-kin, 118-work save loads and remains responsive. A 1800-tick mature run spent 34ms of 2801ms in the new social pass (~1.2%; illustrative local measurement, not an FPS guarantee).

Full original simulation regression ranges 1–62 and 63–124 were launched after the simulation implementation; logs output/life-regression-a/b.log (PowerShell buffers these until completion). See subsequent entry for final results. The only sim edit after those workers began was removing an anticipatory phrase from bead-placement narration. No deployment and no edits to the user's actual browser save.
...
## Organic medieval growth — 2026-09-15 (supersedes the uniform-grid visual rule)

Kyle found v1 too symmetrical and explicitly requested a real medieval town growing. Layout version 2 uses town-layout.js: deterministic curved high street, two oblique side lanes, staggered frontages and variable setbacks. It uses integer hashing and rational curves, no simulation RNG draws or trigonometry. _townSite prefers these frontages; local courtyard infill handles tight coasts and full streets, always checking _plotFits. No mirrored opening or grid snapping remains.

Works persist lane, along, roadX/roadY and faceX/faceY. The view rotates doors toward their lane and fits the rotated silhouette into the same collision reservation. _paintStreets draws occupied lane spans and door paths rather than a rectangular road blanket; tree clearing uses this actual paint. New founding searches a small inland common within six cells per axis of the original hearth to avoid an entire village being forced onto one shoreline row.

_relaxTownLayout migrates pre-v2 saves transactionally, retaining IDs, home assignments, progress and stock. Lightly used artificial L-shaped founding paths from v1 are softened; heavily walked tracks survive. A save that cannot fit every building stays intact. Current saves round-trip without resiting. Historic grid test commands redirect to tools/verify-town-layout.mjs, verify-town-browser.cjs, and verify-town-soak.mjs. Focused tests check 22 asymmetric openings, actual rendered roof clearances, 118-work migration, 26 rotated model/fallback fits, social behavior, browser saves and multi-seed 65-day growth.
